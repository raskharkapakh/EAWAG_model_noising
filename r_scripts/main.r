...
# -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# experiment variables ----
for (std.dev in c(1,3,5)){
  file.input.data         <- "All_2729samples_9envfact_lme.area.elev_ModelInputs.csv"
  file.prev.taxa          <- "All_2729samples_9envfact_lme.area.elev_PrevalenceTaxa.csv"
  
  experiment.name         <- paste0("experiment_temp_",#TODO: change name
                                    std.dev, 
                                    "_",
                                    format(Sys.time(), "%d_%m_%Y_%Hh%M"))
  number.split            <- 3
...
# GAMARIDAE MISSDETECTION NOISE
# -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# experiment variables ----
for (prob in c(10, 25, 50)){
  file.input.data         <- "All_2729samples_9envfact_lme.area.elev_ModelInputs.csv"
  file.prev.taxa          <- "All_2729samples_9envfact_lme.area.elev_PrevalenceTaxa.csv"
  
  experiment.name         <- paste0("experiment_miss_gammaridae_",#TODO: change name
                                    prob,
                                    "_",
                                    format(Sys.time(), "%d_%m_%Y_%Hh%M"))
  number.split            <- 3
...
# -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# experiment variables ----
for (set.size in c(500, 1000, 2000)){
  file.input.data         <- "All_2729samples_9envfact_lme.area.elev_ModelInputs.csv"
  file.prev.taxa          <- "All_2729samples_9envfact_lme.area.elev_PrevalenceTaxa.csv"
  
  experiment.name         <- paste0("experiment_subset_",#TODO: change name
                                    set.size,
                                    "_",
                                    format(Sys.time(), "%d_%m_%Y_%Hh%M"))
  number.split            <- 3
  split.criterion         <- "SiteId"
  number.sample           <- set.size
  models                  <- list("null", "glm", "gamloess", "rf", "ann")
  env.factor              <- c("Temperature"                      = "temperature",       # Temp
                               "Flow velocity"                    = "velocity",          # FV
                               "Riparian agriculture"             = "A10m",              # A10m
